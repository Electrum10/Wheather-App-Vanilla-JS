...
BotonBuscar.addEventListener("click", () => {
  ...
  .then((data) => {
    ...
    let temp = Math.round(data.main.temp);
      ...
      const API_LLUVIA = `https://www.meteosource.com/api/v1/free/point?place_id=${Buscar.value}&sections=all&timezone=UTC&language=en&units=metric&key=${API_KEY_METEOSOURCE}`
      fetch(API_LLUVIA)
      .then((respuesta) => {
        return respuesta.json();
      })
      ...
      const API_7_DIAS = `https://www.meteosource.com/api/v1/free/point?place_id=${Buscar.value}&sections=daily&timezone=UTC&language=en&units=metric&key=${API_KEY_METEOSOURCE}`;
    fetch(API_7_DIAS)
      ...
      QualidadAire.textContent = "Para mostrar por ciudad tengo que pagar. Lo siento :("
      document.getElementById("GraficoQualidadAire").style.display = "none"
      document.getElementById("QualidadAireGlobal").style.gridTemplateColumns = "100%"
    })
